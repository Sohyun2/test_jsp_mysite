	...
	<div id="content">
		<div id="board">
						...
						<tr>
							...
							<c:choose>
								<c:when test="${vo.depth == 0 }">
									<td><a href="${pageContext.servletContext.contextPath }/board?a=detail&page=${page }&no=${vo.no }">${vo.title }</a></td>
								</c:when>
								<c:otherwise>
									<td style="padding-left:${50*vo.depth}px">
										...
										<a href="${pageContext.servletContext.contextPath }/board?a=detail&page=${page }&no=${vo.no }">${vo.title }</a>
									</td>
								</c:otherwise>
							</c:choose>
							...
							<td>
								<c:if test="${vo.userNo eq authuser.no }" >
									<a href="${pageContext.servletContext.contextPath }/board?a=delete&page=${page }&no=${vo.no }" class="del">
			...
			<c:if test="${!empty authuser }" >
				<div class="bottom">
					<a href="${pageContext.servletContext.contextPath }/board?a=insertform&page=${page }" id="new-book">글쓰기</a>
				</div>
			</c:if>					
		</div>
	</div>
